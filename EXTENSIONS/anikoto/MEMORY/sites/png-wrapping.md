...
> Last updated: 2026-06-22 (session 11) · Status: VERIFIED (bytes inspected + reference APK confirmed)
> Affects: ALL working servers (VidPlay-1, HD-1, Vidstream-2, Kiwi-Stream) + ★ ad segments too

**Session-11 confirmation**: the 70-byte PNG header structure is exactly as documented below. The reference APK's `stripPngHeader` algorithm (find IEND, cut at IEND+8, then scan for 0x47@188 alignment) correctly handles this. **Both real AND ad segments are PNG-wrapped** — ad filtering happens at the playlist level (by CDN host), before any segment fetch. See `EXTENSIONS/anikoto/EXTENSIONS/anikoto/MEMORY/research/apk-reference/02-video-pipeline-and-proxy.md` §5.4 for the full algorithm. We implement a custom `LocalProxyServer` (NOT yuzono's `lib/m3u8server`) per ADR 03.
