 ...
 * AnimePahe filters.
 *
 * AnimePahe's filter model: only ONE browse filter is applied at a time (genre OR demographic OR
 * theme OR season). If a search query is present, search is used instead of any filter.
 * See [AnimePahe.searchAnimeRequest] for the resolution order.
 *
 * Filter values (slugs) verified from the reference extension's Filters.kt — these are the
 * URL path segments the site expects (e.g. `/anime/genre/slice-of-life`).
 */
object Filters {

    /** Browse by Genre — `/anime/genre/<slug>` (22 genres). */
    ...
    /** Browse by Theme — `/anime/theme/<slug>` (52 themes). */
    ...
    /** Browse by Demographic — `/anime/demographic/<slug>` (5 demographics). */
    ...
    /** Year filter (used WITH SeasonFilter) — `/anime/season/<season>-<year>`. */
    class YearFilter : UriPartFilter(
        "Browse by Season (Year)",
        YEARS,
    ) {
        companion object {
    ...
    /** Season filter (used WITH YearFilter) — `/anime/season/<season>-<year>`. */
    ...
    /** Base class for select-style filters that map a display name → a URL slug. */
