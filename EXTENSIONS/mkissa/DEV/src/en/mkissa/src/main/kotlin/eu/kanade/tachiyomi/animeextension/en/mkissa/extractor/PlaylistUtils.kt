...
/**
 * Simplified HLS (m3u8) playlist parser — ported from the keiyoushi lib/playlistutils,
 * adapted for ext-lib 16 (named-arg Video constructor).
 *
 * Fetches a master .m3u8 playlist, parses variant streams (#EXT-X-STREAM-INF),
 * and returns one [Video] per variant. Also extracts subtitle tracks (#EXT-X-MEDIA:TYPE=SUBTITLES).
 *
 * For single-stream playlists (no variants), returns one Video pointing at the playlist URL.
 */
class PlaylistUtils(
    ...
    suspend fun extractFromHls(
        ...
        try {
            MKissaLog.d("PlaylistUtils: fetching HLS playlist: ${MKissaLog.trunc(playlistUrl, 100)}")
            val masterHeaders = buildHeaders(referer)
            val response = client.newCall(GET(playlistUrl, masterHeaders)).execute()
            val code = response.code
            val body = response.use { it.body?.string().orEmpty() }
            MKissaLog.d("PlaylistUtils: HLS fetch HTTP $code, body length=${body.length}")
            if (body.isBlank()) {
                MKissaLog.w("PlaylistUtils: empty HLS response (HTTP $code)")
                return@withContext emptyList()
            }
            if (code !in 200..299) {
                MKissaLog.w("PlaylistUtils: HLS fetch failed (HTTP $code) — body: ${body.take(200)}")
                return@withContext emptyList()
            }
